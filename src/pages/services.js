...
const html = `
  ...
  <section class="page-section">
    ...
    <div class="service-grid">
      ${services.map(([num, title, desc, href]) => `<a class="service-card reveal" href="${href}"><span>${num}</span><h3>${title}</h3><p>${desc}</p><span style="position:absolute;right:21px;bottom:19px;color:var(--orange);font-size:22px">↗</span></a>`).join('')}
    </div>
  </section>
...
document.querySelector('#app').innerHTML = renderShell('Services', html)
initShell()
